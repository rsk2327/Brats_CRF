...
from pystruct.models import GraphCRF, LatentNodeCRF, LatentGraphCRF
...
def trainModel_Latent(num_iter=5,latent_iter=3,inference="qpbo",trainer="NSlack",num_train=2,num_test=1,C=0.1,edges="180x180_dist1_diag0",inputs=[1,1,1,1,1,1],features="all",directed=False,savePred=False):
    ...
    if trainer=="Frank":
        base_svm = FrankWolfeSSVM(model = crf,max_iter=num_iter,C=C,n_jobs=-1,verbose=1)
        svm = LatentSSVM(base_ssvm=base_svm)
    elif trainer=="NSlack":
        base_svm = NSlackSSVM(model = crf,max_iter=num_iter,C=C,n_jobs=-1,verbose=1)
...
#    predFileTest.close()   
    ...
    str(np.std(overallDicePerPatient))+","+edgeFeature+","+"None"+","+features+","+filename +","+ str(overallPrec) +","+ str(overallRecall) +","+ str(overallExtDice)+","+"Flair+T2Latent with 2 states"+"\n")
